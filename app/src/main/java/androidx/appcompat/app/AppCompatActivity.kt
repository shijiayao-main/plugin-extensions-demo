 ...
 */
@Replace(name = "appcompat:1.4.2")
open class AppCompatActivity : FragmentActivity, AppCompatCallback, SupportParentable, ActionBarDrawerToggle.DelegateProvider {
    ...
    override fun setContentView(view: View) {
        Toast.makeText(this, "设置了ContentView: " + view.javaClass.simpleName, Toast.LENGTH_LONG).show()
        initViewTreeOwners()
    ...
    override fun onMenuItemSelected(featureId: Int, item: MenuItem): Boolean {
        ...
        return if ((item.itemId == R.id.home) && (ab != null) && (
                    (ab.displayOptions and ActionBar.DISPLAY_HOME_AS_UP) != 0)
        ) {
            onSupportNavigateUp()
        } else false
    }
 ...
 * Back Stack]({@docRoot}guide/topics/fundamentals/tasks-and-back-stack.html) from the developer guide and
    ...
    private fun performMenuItemShortcut(event: KeyEvent): Boolean {
        if ((Build.VERSION.SDK_INT < 26 && !event.isCtrlPressed
                    && !KeyEvent.metaStateHasNoModifiers(event.metaState)
                    && (event.repeatCount == 0
    ...
    companion object {
        private val DELEGATE_TAG = "androidx:appcompat"
    }
}